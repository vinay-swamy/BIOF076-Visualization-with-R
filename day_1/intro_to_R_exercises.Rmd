...
```{r ex_3}
first_Vector <- c(2, 5, 7, 2)
second_vector <- c(3, 4, 523)

final_vector <-  c(first_Vector, second_vector)
final_vector
```
...
```{r ex_6}
indexing_vector <- c(1, 3, 5)
char_vec[indexing_vector]
char_vec[c(1, 3, 5)]
```
...
```{r ex_14}
vector_of_means <-  c(mean(v1) ,mean(v2) ,  mean(v3) )
sum(vector_of_means)
```
...
add_five <- function(number){
    result <- number + 5 
    vector_1 <- c('1')
    print('above the return')
    return(result)
    print('outside the return')
}
...
get_last_element <- function(some_vector){
    last_element_index <- length(some_vector)
    return(some_vector[last_element_index])
}
...
my_list <- list(condition_a=a, condition_b=b)
my_list
```
"
...
```{r ex_18}
names(my_list) <- c('wild_type', 'knock_out')
my_list
```
...
```{r ex_19}
last_index <- length(my_list$wild_type)
my_list$wild_type[last_index] <- -1
my_list
```
...
```{r}
faang_data$location <- c('california', 'california', 'seatle', 'california', 'california')
faang_data
```
...
```{r ex_21}
revenue_per_employee <- faang_data$revenue / faang_data$n_employees
faang_data$revenue_per_employee <- revenue_per_employee
faang_data$name <- c('a', 'b', 'c', 'd', 'e')
faang_data
```
...
first_three_ceos <- faang_data[1:3, 'ceo']
last_names_added <- paste(first_thee_ceos, c('zuckerberg', 'cool', 'bezos') )
  
faang_data$new_column <- c(1, 2, 3, 4, 5)
colnames(faang_data)

faan_data_reorderd <- faang_data[ ,c( "name" ,"new_column", "revenue" ,"n_employees", "ceo", "service", "new_column" )]
