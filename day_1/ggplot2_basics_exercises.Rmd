...
```{r}
ggplot(data = CO2)+
    geom_point(aes(x=conc, y=uptake, color = Plant, shape = Treatment))
```
...
`geom_freqpoly` is  drop-in replacement for `geom_histgram`: instead of drawing bar's for the count in each bin, it draws points at each count, and then draws a line through each point. Try it out with the above data, tweaking the colorign if necessary
...
now plot make violin plots for each location;
