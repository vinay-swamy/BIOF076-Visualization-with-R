...
to selec a continous range of values, we can use the `:` operator. this is called slicing
...
- We have to pass data to functions that is already defined, or that we define on the spot

```{r}
# defined on the sport
add(3, 4)
```
...
add(a, b)
```
- here, R looks for two variables in the environment named `a` and `b`, and uses them as the arguments for add. In both cases, the values given to `add` are assigned to the arguments `num_1` and `num_2`  in the order they are given. 

- We can also explictly set the arguments of a function. When we do this, order does not matter.
...
-columns can also be accesed by there column name 
```{r}
example_df[,'col_b']
```
...
```{r}
example_df_no_fctr <- data.frame(col_a = c(1, 2, 3 ), col_b = c('a', 'b', 'c'), stringsAsFactors = F)
