...
- its easy to include external images within a patchwork style plot as well.
- This code snippet will lets you load an external image as a ggplot
```{r}
library(png)
library(grid)
library(ggplotify)
...
- note that this can sometimes add white space to plot, as the `rasterGrob` function will use all available plotting space in the plotting window, which by default is 7X5. 
