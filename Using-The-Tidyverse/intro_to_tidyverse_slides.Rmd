...
- this function will rename one or mor columns in a data.frame/tibble

```{r}
rename(iris, sepal_length=Sepal.Length)
```

- the syntax is structured <new_name> = <old_name>
...
-  we do this by using the `group_by` and `summarise` functions from `dplyr`
...
```{r echo=F}
example_df <- tibble(condition = rep(c('wt','ko'), 6), 
       timepoint = rep(c('1hr', '2hr', '3hr', '4hr', '5hr', '6hr'), 2) %>% sort, 
       ) %>% bind_rows(.,.,.) %>% 
  mutate(replicate = rep(c('1', '2', '3'), 12) %>% sort, 
         measurement = rnorm(36, 10, 2)) %>% 
  sample_n(36, replace = F)
```
```{r}
...
- when we use `group_by`, we create "sub-dataframes", based on common values in the grouping column(s)

```{r, echo = F}
example_df %>% arrange(condition) %>% datatable()
```

- We never actually see these "sub-dataframes" as it happens under the hood.

## grouping data

- `group_by` does nothing on its own; it always requires a second command to perform an operation on the groups
...
- we can group on any number of columns

```{r,echo = F}
example_df %>% arrange(condition, timepoint) %>% datatable()
```

## summarizing data

- `summarise` takes in the output of `group_by`, and performs operations on grouped columns

```{r}
example_df %>% group_by(condition) %>% summarise(total_measurment = sum(measurement))
```

- In this example, all values from the `measurement` column that belong to the `wt` group are added to together, and similar for `ko`
- in previous version of dplyr(<1), it was required that there must be one value produced per group, but newer versions(>=1) allow for multiple values to be generated per group( the output for multiple values will be in long form - more on that in a few slides )
- most often, you will be producing a 1:1 mapping between groups and summarised output
...
- same thing a different way, using the **negative selection syntax.**
- we use the negative selection syntax to drop columns we *dont* want to pivot
- this is useful when we have many columns that we want to pivot, but don't want to type them all out
