...
## An aside on theme's and graphical parameters

- while there is a bit of consistency on how to use and change different graphical parameters, much of it will feel a little arbitrary, and there's no good way to learn how to change them with out trial and error/practice 
...
- there are various setting for text - font, size color etc; These are controlled by the `element_text` function, the output of which we assign to to specific graphical parameter, like `axis.text`. 
- these parameters change everything about the text, except the text itself; setting the text is done with functions like `xlab` or `ggtitle` or populated directly from the data
...
- They were introduced to ggplot after themes, and are generally an easier to use interface for setting graphical parameters, but lack the full range of parameters offered by `theme`
