...
```{r ex_7}
ggplot(data = CO2 ) +
  geom_point(aes(x=conc, y = uptake,shape = Treatment, color = Plant))
```
...
ggplot(data = CO2) +
  geom_point(aes(x=conc, y = uptake), alpha =.75)
```
...
```{r ex_11}
ggplot(data = Indometh) + 
  geom_line(aes(x=time, y=conc, color = as.character(Subject)))
...
```{r ex_15}
ggplot(mpg)+
  geom_col(aes(x=class, y=hwy ,fill = class))
...
```{r ex_20}
ggplot(ToothGrowth) + 
  geom_boxplot(aes(x=supp, y=len, fill = supp), width = .5) +
  theme_minimal()
```
...
ggplot(hospital_data) + 
  geom_boxplot(aes(x=Hospital_location, y=age))
```
...
```{r ex_22}
ggplot(hospital_data) + 
  geom_violin(aes(x=Hospital_location, y=age))
```
...
```{r ex_24}
ggplot(hospital_data) + 
  geom_freqpoly(aes(x=age, color = Hospital_location))
```
