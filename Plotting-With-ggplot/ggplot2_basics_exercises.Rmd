...
```{r}
ggplot(data=CO2) +
  geom_point(aes(x=conc, y=uptake, shape = Treatment ,color = Type), alpha = .25) +
  theme_minimal()
...
```{r}
ggplot(data = Indometh) + 
  geom_smooth(aes(x=time, y=conc, color =Subject), se = F) +
  theme_classic()
...
```{r}
ggplot(hospital_data) + 
  geom_violin(aes(x=Hospital_location, y=age, color = Hospital_location))
```
...
```{r}
ggplot(hospital_data) + 
  geom_histogram(aes(x=age, fill = Hospital_location ), bins = 100)
...
```{r}
ggplot(hospital_data) + 
  geom_freqpoly(aes(x=age, color = Hospital_location ), bins = 100)
```
