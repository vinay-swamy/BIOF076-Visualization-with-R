...
- I will use the terms `library` and `package` interchangeably
...
- If you see this error, it almost always means you have not loaded the package the function is from. (or you spelled something wrong)
...
- Almost all libraries are open source and publicly hosted. The largest package repository is CRAN(Comprehensive R Archive Network). We can download packages from CRAN using the `install.packages` command

```{r}
install.packages('aricode', repos = "https://cloud.r-project.org")
```

- You only need to run this command once; The package is downloaded and stored on your computer
...
- horizontal and verticals lines can be drawn with `geom_hline` and `geom_vline` respectively. As these are just single lines, they don't need data attached to it
```{r}
ggplot() + 
...
-  When visualizing continuous data, there are several metrics we often want to (visually) assess such as:
    - mean
    - skew 
    - variance 
    - modality 

- We will cover several plots for visualizing continoius data in the n ext few slides 
...
- histograms are much better ar visualizing the distribution of data. Histograms are generated by splitting series of numeric values into bins, calculating the number of observations in that bin, and plotting a bar for the count of each bin
...
- More bins makes higher resolution plots; the look of a histogram is heavily dependent on the number binn; a poor choice in the number of bins may lead to misleading plots.
...
- a "modern" version of the histogram. Instead of counting the observations in bins, a mathematical function is fit the data, and the area under the curve represents relative proportion that interval makes up of the total dataset

```{r}
ggplot(hist_data) + 
    geom_density(aes(x=value), fill = 'lightblue') + 
  theme_minimal()
```

- like histograms, they are not useful for comparing multiple sets of continuous data
...
- Generally, v violin plots are much more informative than box plots, and its almost always better to use a violin pot over a box plot. 
