...
ggplot(ToothGrowth) + 
    geom_violin(aes(x=supp, y= len, fill = supp), color = 'white') + 
    geom_boxplot(aes(x=supp, y=len),width=.2, fill='black', color  = 'white') + 
    theme_classic()
```
...
data <- read_tsv('../src/Reporter_assay_results.tsv.gz') %>% 
    pivot_longer(cols = c(WildType,Mutant), names_to = 'condition', values_to = 'value') %>% 
    group_by(Time, condition) %>% 
    summarise(avg_val = mean(value), 
              std_dev = sd(value), 
              n_obs = n()) %>% 
    mutate(std_err = std_dev / sqrt(n_obs))

shape_map <- c("Mutant" = 16,"WildType" = 18)
label_df <- data.frame(x=13,
           y= 12, 
           label = "Reaction temp decreased")
ggplot() + 
    geom_vline(xintercept = 8, color  ='red')+
    geom_errorbar(data = data, aes(x =Time, ymin = avg_val - std_err, ymax = avg_val  + std_err ), 
                  width = .4)+
    geom_point(data = data, aes(x=Time, y=avg_val, color = condition, shape = condition), size = 3) + 
    geom_line(data = data, aes(x=Time, y=avg_val, color = condition)) +
    geom_label(data = label_df, aes(x=x, y=y, label=label)) + 
    scale_shape_manual(values=shape_map) + 
    ggtitle('Enzymatic Activity') + 
...
```{r ex_4}
library(tidyverse)
db_data <- read_csv('../src/health.csv.gz') %>% 
    pivot_longer(cols = c(pct_2013, pct_2014),
                 names_to = 'year', 
                 values_to = 'price') %>% 
    arrange(desc(price) ) %>% 
...
col_map = c('pct_2013' = 'blue', 'pct_2014' = 'red')

ggplot(db_data) + 
    geom_line(aes(x=price, y=Area), color = 'red') + 
    geom_point(aes(x=price, y =Area, color =  year)) + 
